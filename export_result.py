...
def export_result_database_to_sql():
    with app.app_context():
        ...
        # Table to export
        table = "result"  # Replace with your table name

        # Path where the backup will be saved
        backup_file_path = "/home/gigo/AAIS/export_result_backup.sql"
        ...
        with open(backup_file_path, "w") as f:
            ...
            if rows:
                # Retrieve column names to identify datetime fields
                cursor.execute(f"DESCRIBE {table}")
                columns = [col[0] for col in cursor.fetchall()]
                datetime_columns = ["created_at", "date_issued"]  # Add any datetime fields
                ...
                for row in rows:
                    ...
                    for datetime_col in datetime_columns:
                        if datetime_col in row_dict and row_dict[datetime_col]:
                            if isinstance(row_dict[datetime_col], datetime):
                                row_dict[datetime_col] = row_dict[datetime_col].strftime("%Y-%m-%d %H:%M:%S")
                            else:
                                row_dict[datetime_col] = str(row_dict[datetime_col])
