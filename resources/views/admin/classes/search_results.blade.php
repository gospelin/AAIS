...
@section('title', 'Search Results for Students in {{ $class->name }}')
@section('description', 'Search results for students in {{ $class->name }} at Aunty Anne\'s International School.')

@push('styles')
    <style>
        /* Reuse styles from students_by_class.blade.php */
        .content-container {
            max-width: 90rem;
            margin: 0 auto;
    ...
    <div class="content-container">
        ...
        @else
            ...
            <div class="filter-section">
                <h3 class="filter-header">Search Results for {{ $class->name }} ({{ $selectedSession->year }} -
                    {{ $currentTerm->value }})</h3>
                <form id="filterForm"
                    action="{{ route('admin.search_students_by_class', ['className' => urlencode($class->name), 'action' => $action]) }}"
                    method="GET">
                        ...
                        <div class="form-group">
                            ...
                            <select name="term" id="term" class="form-select">
                                @foreach($termChoices as $term)
                                    <option value="{{ $term->value }}" {{ $currentTerm->value === $term->value ? 'selected' : '' }}>
                                        {{ $term->label }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @else
                    <table class="student-table">
                        ...
                        <tbody id="students-table-body">
                            @include('admin.classes.pagination')
                        </tbody>
                    </table>
                    <div class="pagination" id="paginationLinks">
                        {{ $students->appends(['session_id' => $selectedSession->id, 'term' => $currentTerm->value, 'enrollment_status' => $enrollmentStatus, 'fee_status' => $feeStatus, 'approval_status' => $approvalStatus, 'query' => $query])->links('vendor.pagination.bootstrap-5') }}
                    </div>
                @endif
            </div>
                    ...
                    <form id="promotionForm" method="POST" action="">
                        ...
                        <div class="form-group">
                            ...
                            <select name="promotion_session_id" id="promotion_session_id" class="form-select">
                                @if($selectedSession)
                                    <option value="{{ $selectedSession->id }}">Current Session ({{ $selectedSession->year }})</option>
                                    @if($nextSession)
                                        <option value="{{ $nextSession->id }}">Next Session ({{ $nextSession->year }})</option>
                                    @else
                                        <option value="" disabled>Next session not available (create in <a href="{{ route('admin.manage_academic_sessions') }}">Manage Sessions</a>)</option>
                                    @endif
                                @else
                                    <option value="" disabled>No session available (set in <a href="{{ route('admin.set_current_session') }}">Set Current Session</a>)</option>
                                @endif
                            </select>
                        </div>
                        <div class="form-group">
            ...
            function attachPromotionModalListeners() {
                document.querySelectorAll('.open-promotion-modal').forEach(button => {
                    button.addEventListener('click', () => {
                        ...
                        const promotionSessionSelect = document.getElementById('promotion_session_id');
                        promotionSessionSelect.value = action === 'promote' && '{{ $nextSession->id ?? '' }}' ? '{{ $nextSession->id }}' : sessionId;
                        ...
                        fetch(suggestUrl, {
                            ...
                            .then(data => {
                                ...
                                gsap.from('.modal-content', { opacity: 0, scale: 0.8, duration: 0.3 });
                            })
                            .catch(error => {
                                loadingOverlay.style.display = 'none';
                        ...
                        if (url) {
                            ...
                            fetch(url, {
                                ...
                                .then(data => {
                                    ...
                                    loadingOverlay.style.display = 'none';
                                    gsap.from('#students-table-body tr', { opacity: 0, y: 20, stagger: 0.1, duration: 0.6 });
