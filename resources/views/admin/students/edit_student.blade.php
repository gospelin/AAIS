{{-- resources/views/admin/students/edit_student.blade.php --}}
...
@section('content')
    <div class="content-container">
        <div class="form-container">
            <h2 class="form-header">Edit Student: {{ $student->full_name }}</h2>
            <form method="POST" action="{{ route('admin.students', ['action' => 'edit', 'student' => $student->id]) }}" id="editStudentForm">
                @csrf
                @method('POST')
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <input type="text" name="parent_name" id="parent_name" class="form-control @error('parent_name') is-invalid @enderror" 
                                   value="{{ old('parent_name', $student->parent_name) }}" placeholder="Enter parent/guardian name" required>
                            @error('parent_name')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="tel" name="parent_phone_number" id="parent_phone_number" class="form-control @error('parent_phone_number') is-invalid @enderror" 
                                   value="{{ old('parent_phone_number', $student->parent_phone_number) }}" placeholder="Enter phone number (e.g., +234xxxxxxxxx)" required>
                        ...
                        <div class="form-group">
                            ...
                            <select id="class_select" class="form-control form-select" required>
                                ...
                                @foreach($classChoices as $class)
                                    <option value="{{ $class->id }}" {{ $student->getCurrentClass() == $class->name ? 'selected' : '' }}>{{ $class->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
