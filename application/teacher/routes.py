...
from ..models import Student, User, Subject, Result, Session, StudentClassHistory, RoleEnum, Classes, Teacher
...
def not_found_error(error):
    ...
    return redirect(url_for("teachers.admin_dashboard"))
...
def teacher_dashboard():
    app.logger.info(f"Teacher dashboard accessed by user {current_user.username}")
    
    # Retrieve the logged-in teacher
    teacher = Teacher.query.filter_by(employee_id=current_user.username).first()
    if not teacher:
        flash("Teacher profile not found.", "alert-danger")
        return redirect(url_for("main.index"))

    # Initialize forms
    profile_form = TeacherForm(obj=teacher)
    subject_form = AssignSubjectsForm()
    class_form = AssignClassesForm()

    # Handle profile updates
    if profile_form.validate_on_submit():
        profile_form.populate_obj(teacher)
        db.session.commit()
        flash("Profile updated successfully.", "alert-success")
        return redirect(url_for("teachers.teacher_dashboard"))

    # Retrieve assigned subjects and classes
...
# Route to assign subjects to teacher
@teacher_bp.route('/assign_subjects', methods=["GET", "POST"])
@login_required
def assign_subjects():
    teacher = Teacher.query.filter_by(employee_id=current_user.username).first()
    ...
    if subject and subject not in teacher.subjects:
        ...
        db.session.commit()
        flash("Subject assigned successfully.", "alert-success")
    else:
        flash("Subject not found or already assigned.", "alert-danger")

    return redirect(url_for('teachers.teacher_dashboard'))

@teacher_bp.route("/remove_subject/<int:subject_id>", methods=["GET", "POST"])
@login_required
def remove_subject(subject_id):
    ...
    if subject in teacher.subjects:
        ...
        db.session.commit()
        flash(f"Removed subject {subject.name} successfully.", "alert-success")
    else:
        flash(f"Subject {subject.name} not found in your assignments.", "alert-danger")
    return redirect(url_for("teachers.teacher_dashboard"))


@teacher_bp.route("/assign_classes", methods=["GET", "POST"])
@login_required
def assign_classes():
    teacher = Teacher.query.filter_by(employee_id=current_user.username).first()
    if not teacher:
    ...
    assigned_class = Classes.query.get_or_404(class_id)
    if assigned_class not in teacher.classes:
        teacher.classes.append(assigned_class)
        db.session.commit()
        flash(f"Assigned class {assigned_class.name} successfully.", "alert-success")
    else:
        flash(f"Class {assigned_class.name} is already assigned.", "alert-warning")
    return redirect(url_for("teachers.teacher_dashboard"))


@teacher_bp.route("/remove_class/<int:class_id>", methods=["GET", "POST"])
@login_required
def remove_class(class_id):
    ...
    assigned_class = Classes.query.get_or_404(class_id)
    if assigned_class in teacher.classes:
        teacher.classes.remove(assigned_class)
        db.session.commit()
        flash(f"Removed class {assigned_class.name} successfully.", "alert-success")
    else:
        flash(f"Class {assigned_class.name} not assigned.", "alert-danger")
    return redirect(url_for("teachers.teacher_dashboard"))
...
# #                 last_term_average, 
# #                 subjects_offered
# #             ) = calculate_results(student_id, current_term, current_session.year)
